...
-- View/Table Name: acad_usage_summary
...
BEGIN 
	...
	IF EXISTS (
		SELECT
			DISTINCT event_date
		FROM analytics_rpt.acad_usage_summary
		WHERE
			event_date > (CURRENT_DATE - 4)
	) 
	THEN 
		DELETE FROM analytics_rpt.acad_usage_summary
		WHERE
			event_date > (CURRENT_DATE - 4);
	END IF; 
	
	INSERT INTO analytics_rpt.acad_usage_summary (
		WITH ga4_event AS(
			SELECT
				customer
				,event_date
		...
		LEFT JOIN searches search
			...
			AND ga4.event_date = search.timestamp_date
